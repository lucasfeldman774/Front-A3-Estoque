...
import { criarCategoria, listarCategorias, atualizarCategoria, excluirCategoria } from './categoriaService'
...
export function CategoriasPage() {
  ...
  const editarMut = useMutation({
    mutationFn: () => atualizarCategoria(editId!, { nome: editNome, tamanho: editTamanho as Tamanho, embalagem: editEmbalagem as Embalagem }),
    onSuccess: () => {
      setEditId(null)
      qc.invalidateQueries({ queryKey: ['categorias'] })
    },
              ...
              {editId === c.id && (
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <select className="select" value={editTamanho} onChange={(e) => setEditTamanho(e.target.value as Tamanho)}>
                      <option value="" disabled>Tamanho</option>
                      <option value="PEQUENO">PEQUENO</option>
                      <option value="MEDIO">MEDIO</option>
                      <option value="GRANDE">GRANDE</option>
                    </select>
                    <select className="select" value={editEmbalagem} onChange={(e) => setEditEmbalagem(e.target.value as Embalagem)}>
                    ...
                    <button
                      ...
                      disabled={editarMut.isPending || !editNome || !editTamanho || !editEmbalagem}
                    >
                      Salvar
                    </button>
                    <button
                  ...
                  {editarMut.error && (
                    <p className="text-sm text-red-400 bg-red-900/20 border border-red-500/40 rounded p-2">{String(editarMut.error)}</p>
                  )}
                </div>
              )}
            </li>
